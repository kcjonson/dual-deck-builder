...
**How:** 20 tests in `EffectTargets.test.ts` (Berserker at low gunnery, EMP Blast from both sides with per-target misses and an out-of-fight raider, mixed self and target cards, projection against play, an area card's adrenaline gained once, a draw after the target is wrecked, a both_drivers card) and 18 card data checks. The Berserker Rage test in `Battle.test.ts` now expects the self damage it used to mark as a known gap.
