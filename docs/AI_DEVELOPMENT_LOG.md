...
- Card data: `order` tag (never `attack`), `escort` target type, `signatureOf` with `signature` rarity, `spendsEscort`, and `broughtBy` on a copy. `CardLoader` validates all of it. Effects gained `on_hit`, `structure_only`, `grant_adrenaline`, and `draw_fire`; formula damage now adds to the printed value (Ram's is 0, so it's unchanged).
...
**How:** 51 tests in `Orders.test.ts` (card data, tie order, the selector's ranges, ties, spent, and no-carrier cases, a passenger playing an order, the reset, and each card), 5 new `CardLoader` checks, and the card data check covering the new summaries (44 to 59 rendered against 60). Checked in the dev server with escorts and orders patched into the combat screen's opening (not committed): Covering Fire lit the Outrider over the Rust Buggy and spent it, Ramming Run left the Buggy untargetable, Draw Fire lit only escorts, Run Ahead put the Outrider on the shoulder, and SPENT cleared on turn 2. The combat screen golden is unchanged by default; the card browser golden will need a CI re-baseline for the new cards.
