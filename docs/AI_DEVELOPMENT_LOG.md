...
- Conversion: `Team.handleDriverDeath(vehicle)` runs after every driver hit (`Battle.damageVehicle`, `damageDriver`). On the player's team a living vehicle with nobody at the wheel goes through `convertToEscort` (`Escort.ts`): profile type null, `UNMANNED_CREW` (gunnery 4, evade 3, ramming 2), role gun, no signature card or dividend, preferred slot = the formation slot it held, and `spent` true; `readyEscorts` readies it at the next player turn, so an enemy-turn conversion is ready. `EscortProfile.type` and `signatureCard` are nullable for it.
