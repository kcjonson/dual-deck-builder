...
- Fight state is cleared at the end of the fight, not on entry (PR #50 review note): `Vehicle.leaveFight` clears slot, flank, statuses, Shield, spent, and the passenger seat, and refills armor, so an escort that ended on the raiders' shoulder opens the next fight in its preferred slot. Armor refilling is a build call, flagged in escorts.md for Kevin.
- `EscortDividend` is `{ kind: 'fuel' | 'scrap' | 'heal', amount }`. Heals land on the drivers who fought, crashed out included; fuel goes in the result for the run. Nothing pays after a lost fight. Salvage Rig isn't built, since it has no signature card.

**How:** 19 tests in `Convoy.test.ts` (roster order and cap, dismiss, a lost escort's copies from every pile after it's cleared off the road, the copy staying playable while another of its type lives, structure carrying into a new battle, the road-state reset and the flanker re-opening in formation, set-piece allies left out, the conversion case past four via a stand-in for DDB-152's conversion, each dividend, crashed-out and dead drivers, duplicates, a lost fight, and a won fight end to end). Nothing here shows in the app: the combat screen fields no escorts and there's no screen after a fight that reads the convoy, so there was nothing to check in the dev server.
