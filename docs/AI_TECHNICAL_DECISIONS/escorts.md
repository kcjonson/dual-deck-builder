...
## Passengers and unmanned (DDB-152, 2026-09-26)

Built with DDB-152, which seats wreck survivors in escorts and turns a player vehicle that loses its last driver into one. Decisions 10, 11, 16, 17, and 22 and the review fixes were Kevin's; everything below is a build call, written into Combat Rules where it matters, and small enough to change. Numbering starts at 40 to stay clear of DDB-150 and DDB-151.

40. **Escape order.** On the player's team a survivor takes a free seat in the partner's driven vehicle (any vehicle on the team that isn't an escort and has a living driver, which is only ever the partner's), then in the escort nearest the wreck, by range from the wreck's slot with the attack-order ties (`nearestTo` and `compareTieOrder` in `Road.ts`). The driver goes first, then the passenger, each picking from the seats left. A raider still takes the first free seat in team order, near or far. `Team.handleDriverEscape({ driver, from })` is the rule, and `handleVehicleDestruction` returns where everyone went so the battle log can say "jumps from the Rig into the Outrider" or "has no free seat and crashes out of the fight".
41. **Default crew stats: gunnery 4, evade 3, ramming 2** (`UNMANNED_CREW` in `Escort.ts`). A content number, as the spec left it. Gunnery and evade sit below both gun escorts (Outrider 6/6, Pilot Car 5/4) and ramming matches the Outrider's 2, so a converted vehicle is a weak escort, not a free Pilot Car. Against the combat screen's Rust Buggy (evade 4) its Covering Fire misses; against evade 3 or less it hits.
42. **The converted profile.** Type null (it isn't a hired type, so no signature card can name it and nothing hires or dismisses it as one), role gun (it isn't a hauler, so it's not what looters prefer), no signature card, no dividend, not a set piece, and its preferred slot is the formation slot it held, or had reserved while flanking. It keeps its name, armor, structure, and base speed. It's in `Team.escorts` from then on, and as the newest escort it goes to the end of the roster, after every escort already in the convoy; two conversions go in the order they happened. `Team.handleDriverDeath` moves it to the end of `Team.vehicles`, so Rally the Convoy fires it last, and DDB-151's convoy appends it in the same place after the fight. The convoy cap of four is only checked when a vehicle joins, so a full convoy plus a converted vehicle is legal mid-fight; whether it counts toward four afterwards is DDB-151's.
43. **Starts spent, always.** A vehicle only converts during a turn, so `convertToEscort` sets `spent`. One that converts on the enemy turn is readied with every other escort at the start of the player's turn, which is the "turn boundary starts ready" case; one that converts on the player's turn (Berserker, say) sits out the rest of that turn.
44. **Where conversion lives.** `Team.handleDriverDeath(vehicle)`, since losing a driver is scoped by team: it runs `Vehicle.handleDriverDeath` (clear the dead, promote a living passenger) and then, on the player's team, converts a living vehicle with nobody at the wheel. `Battle` calls it after every hit that can kill a driver (`damageVehicle`, `damageDriver`). `Vehicle.isUnmanned` and `isOutOfFight` stay for raiders; in play a player vehicle never stays unmanned, and `clearWrecks` only ever removes the player's wrecks. The DDB-156 stand-in path for the player is gone.
45. **A wreck survivor can ride in a converted vehicle.** It's an escort with a free seat, so DDB-156's crash-out for a survivor whose only open seat was an empty player vehicle no longer happens: they ride in it, as a passenger in an escort.
46. **Raider plans against a converted vehicle.** A card planned at a vehicle whose driver has since died lands on it as an escort, all on structure; a planned Headshot fizzles, since it's an empty escort (decision 22). Draw Fire doesn't cover it, because it's no longer a driven vehicle.
47. **The plate.** `ui/Vehicle.ts` rebuilds its elements when a vehicle's shape (driver or not, escort or not) changes, so a converted vehicle loses the driver row and gains the escort's SPENT chip without a new plate. Passengers still don't show on plates; that's the passenger row noted under DDB-149.
...
- Code that conflicts with these rules today:
  ...
  - `Team.handleDriverEscape` put the driver in the first vehicle with a free seat, not the partner's vehicle and then the nearest escort. DDB-152 fixed it for the player's team (decision 40).
  - `Vehicle.handleDriverDeath` was never called. DDB-156 wired passenger promotion, and DDB-152 added the player's conversion to an escort (decision 44).
