...
- **Shield in code.** `Vehicle.shield` (unset means 0), `addShield`, and `takeDamage` taking Shield before armor; `damageStructure` and driver damage never touch it. `Battle.clearShields` runs on both teams beside `readyEscorts`, at `start` and each player turn. The damage log lists "to shield" and adds a Shield before and after when there was any. The preview's per-hit number is worked out before armor, so Shield doesn't change it, and the projection doesn't model damage, so planning needed nothing. Plates show Shield as "+N" after the armor number.
