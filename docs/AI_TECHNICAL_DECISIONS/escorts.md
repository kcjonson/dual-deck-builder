...
- **Preference fallback.** A raider with a target preference picks its preferred target when that target is legal for the card, and otherwise plans as it does today. DDB-150 built it and assigned the archetypes (see Raider preferences). AI System Technical Design 2.1 points here.
...
Edges the answers didn't cover, noted rather than decided, each left to its build task:
...
The third edge, whether Draw Fire pulls an area intent, was settled with DDB-150 (decision 29).
...
## Raider preferences (DDB-150, 2026-09-26)

Built with DDB-150, which put decision 4's looters and killers into raider planning. Kevin delegated both calls below; each is written into Combat Rules (Enemy intents) and AI System Technical Design 2.1.

29. **Draw Fire and area intents** (decided 2026-09-26, Kevin delegated the call). Draw Fire doesn't redirect an area intent. An intent aimed at both driven vehicles hits both as planned, even with an escort drawing fire in one of their rows, and the escort takes only the hit the blast gives it anyway. Draw Fire pulls aimed fire; it can't pull a blast. Why: the rule stays readable (one aimed card, one redirect), and it's what DDB-149 built.
30. **Archetype assignments** (decided 2026-09-26, Kevin delegated the call). The guideline: a salvage- or loot-flavored raider is a looter, an aggressive or assassin-flavored one is a killer, and a generic one has no archetype. The only raider in the game is the combat screen's test raider:
    - **Rust Buggy (Wasteland Raider): looter.** A buggy built out of rust is scavenger work, the Rust Vultures of Faction Concepts, who disable vehicles to strip them. It's also the one raider a player meets, and a killer would look the same as no archetype there, since the planners already aim at a driver unless an escort is the softer target. A looter is the preference a player can actually see, as soon as a hauler is on the road.

    The battle screen mock's other raiders aren't built. Proposed for when they are: Chain Hauler and Scrapyard Juggernaut looters (towing and scrap), Spike Runner a killer (fast and built to hurt), Dust Crawler none (generic), and Warlord's Pride left for boss design.
