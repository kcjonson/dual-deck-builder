...
- **Armor refills between fights.** Only structure carries over; armor is back to full at the start of the next fight. The spec says an escort "starts the next fight with the structure it ended on", and the garage's Convoy strip repairs structure only, so persisting armor would leave the Pilot Car's or Fuel Hauler's armor gone for the run with nothing to fix it. Flipping it is one line in `Battle.endCombat`. It's escorts only; a driven vehicle's armor and structure between fights are left as they end, for DDB-166.
- **Where the fight's state is cleared: at the end of the fight.** `Battle.endCombat` calls `Vehicle.leaveRoad` on every living player vehicle, driven ones included: slot, flank, statuses, Shield, and spent are cleared. Armor refills on convoy escorts only (the call above). Seats are left alone: a passenger stays where they rode. An escort between fights looks like a fresh one from `createEscort` apart from its structure and anyone aboard, so `Battle.placeOpeningFormation` can keep reading a preset slot as the encounter's, and an escort that ended the fight on the raiders' shoulder opens the next one in its preferred slot instead of throwing; so does a driven vehicle when the same team starts another fight. Clearing on entry instead would mean the battle couldn't tell a leftover slot from an encounter's. This fixes the PR #50 review note.
...
- **Dividends.** Every living convoy escort with a dividend pays at `endCombat`, after a won fight only (decided 2026-09-26 in review): a crashed-out driver is picked up only after a win, which is decision 32's premise, and a loss ends the run. A tie at the turn limit pays nothing. A wrecked hauler pays nothing. `EscortDividend` is `{ kind: 'fuel' | 'scrap' | 'heal', amount }`. The battle applies a heal itself, since it has the drivers who fought. Nothing holds fuel or scrap yet (the combat screen's fuel 5 and scrap 150 are placeholders), so those payouts are in the result for the run layer to add.
...
- Run-state work left for DDB-166: a driven vehicle's armor and structure between fights (it leaves the road at `endCombat` but keeps whatever it ended with), and where each driver sits for the next fight. `endCombat` leaves every seat as it is, so a passenger riding in an escort or in the partner's vehicle is still there afterwards, with role passenger.
